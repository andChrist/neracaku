...
class Database
{
     ...
     * <br />Key[0] : idModal, namaPemodal, namaPkl, besarModal
     * @return array mysql_result. 
     */
    ...
    /**
     * Untuk fasilitator melihat semua user.
     * <br />Key[0] : tipeUser, nama, email, status
     * @param string tipe, antara PKL, Pemodal atau PKL_Pemodal
     * @param string nama user
     * @param string email
     * @param string status_user
     * @return array mysql_result. 
     */
    public function listUser($tipe = false, $nama = false, $email = false, $status = false)
    {
        $condition = array();
        $query = "select tipeUser, nama, email, if(isActive, 'Aktif', 'Non-aktif') as status from user where 1 ";
        if($nama != false){
        ...
        if($tipe != false){
            $tipes = explode('_', $tipe);
            foreach($tipes as $t){
                $condition[] = 'tipeUser ="'. $t.'"';
            }
        }
